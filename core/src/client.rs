...
fn estimate_offset(socket :&mut UdpSocket, ping_addr: &SocketAddr) -> Result<u32> {
    ...
    for i in 0..COUNT {
        ...
        crossy_send(&ping, socket, ping_addr);
...
fn connect(socket: &mut UdpSocket, addr : &SocketAddr, ping_addr : &SocketAddr) -> Result<(ServerTick, u32, game::PlayerId, Instant)>
{
    println!("Connecting..");
...
impl Client
{
    pub fn try_create(port : u16) -> Result<Self>
    {
        ...
        let (server_tick, seed, local_player_id, time_start) = connect(&mut socket, &server, &server_ping)?;
