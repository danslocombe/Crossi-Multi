...
fn is_safe_inner(coordpos : &CoordPos, game_state : &GameState, map : &Map, draw_state : &mut AIDrawState) -> bool {
    ...
    match &current_row.row_type {
        RowType::River(_)  => {
            if let Some(_lillipad) = map.lillipad_at_pos(game_state.get_round_id(), game_state.ruleset_state.get_river_spawn_times(), game_state.time_us, coordpos.to_precise()) {
                true
            }
            else {
                false
