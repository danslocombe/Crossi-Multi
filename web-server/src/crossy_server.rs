...
impl Server {
    ...
    async fn receive_updates(&self, tick_start: &Instant) -> (Vec<RemoteInput>, Vec<game::PlayerId>, Vec<game::PlayerId>) {
            ...
            match message {
                CrossyMessage::Hello(hello) => {
                    ...
                    let client_offset_us =
                        tick_start.saturating_duration_since(inner.start).as_micros() as u32
                            - hello.latency_us;
        ...
        (client_updates, new_players, dropped_players)
    }
    ...
    pub async fn run(&self) {
        loop {
            ...
            let (client_updates, new_players, dropped_players) = self.receive_updates(&tick_start).await;
